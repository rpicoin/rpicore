...
*Setup instructions for a gitian build of RPICoin using a Debian VM or physical system.*
...
- [Building RPICoin](#building-RPICoin)
...
Clone the git repositories for RPICoin and gitian and then checkout the RPICoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the RPICoin repository
...
To build RPICoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the RPICoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/RPICoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/RPICoinProject/core
...
For example:
```bash
URL=https://github.com/crowning-/RPICoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit RPICoin=${COMMIT} --url RPICoin=${URL} ../RPICoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit RPICoin=${COMMIT} --url RPICoin=${URL} ../RPICoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit RPICoin=${COMMIT} --url RPICoin=${URL} ../RPICoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/RPICoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/RPICoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/RPICoin-build.assert
```
...
[RPICoin/gitian.sigs](https://github.com/RPICoinProject/gitian.sigs/) repository, or if that's not possible to create a pull
request.
