...
#include "ui_zrpicontroldialog.h"
...
std::set<std::string> ZRpiControlDialog::setSelectedMints;
std::set<CMintMeta> ZRpiControlDialog::setMints;

bool CZRpiControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZRpiControlDialog::COLUMN_DENOMINATION || column == ZRpiControlDialog::COLUMN_VERSION || column == ZRpiControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZRpiControlDialog::ZRpiControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZRpiControlDialog),
    model(nullptr)
{
    ui->setupUi(this);
    setMints.clear();
...
ZRpiControlDialog::~ZRpiControlDialog()
{
    delete ui;
}

void ZRpiControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRpiControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZRpiControlWidgetItem* itemDenom(new CZRpiControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZRpiControlWidgetItem *itemMint = new CZRpiControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zrpiTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zrpiTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zRPI.";
            else if (!mint.isSeedCorrect)
                strReason = "The zRPI seed used to mint this zRPI is not the same as currently hold in the wallet";
            else
...
void ZRpiControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRpiControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZRpi_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRpiControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZRpiControlDialog::GetSelectedMints()
{
...
void ZRpiControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
