...
unsigned int nStakeMinAge = 10 * 60 * 60; // 8 hours
...
{
    int64_t nSubsidy = 2571336 * COIN; //premine 15% 
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    pindexNew->bnStakeModifierV2 = ComputeStakeModifier(pindexNew->pprev, IsProofOfWork() ? hash : vtx[1].vin[0].prevout.hash);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // Check timestamp
    if (GetBlockTime() > FutureDrift(GetAdjustedTime(), 251))
