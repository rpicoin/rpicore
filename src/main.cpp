...
const std::string strMessageMagic = "Rpicoin Signed Message:\n";
...
CBitcoinAddress addressExp1("RXkJqboCmE2mEK4on75umHuejnw3DMpZR1");
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex* const pindexPrev)
{
    ...
    if ((block.GetBlockTime() <= pindexPrev->GetMedianTimePast()  || (FutureDrift(block.GetBlockTime(), nHeight) < pindexPrev->GetBlockTime() && nHeight < Params().NEW_PROTOCOLS_STARTHEIGHT())) && (Params().NetworkIDString() != "regtest")) {
