...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric) The RPICoin address is the key, the numeric amount in WSP is the value\n"
            "      ,...\n"
            "    }\n"
...
UniValue mintzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. amount      (numeric, required) Enter an amount of Wsp to convert to zRPI\n"
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    ...
    return DoZrpiSpend(nAmount, fMintChange, fMinimizeChange, nSecurityLevel, vMintsSelected, address_str);
}
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "\nSpend zRPI mints to a WSP address.\n" +
            HelpRequiringPassphrase() + "\n"
    ...
    return DoZrpiSpend(nAmount, false, true, 100, vMintsSelected, address_str);
}


extern UniValue DoZrpiSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, const int nSecurityLevel, vector<CZerocoinMint>& vMintsSelected, std::string address_str)
{
