...
#include "lightzrpithread.h"
#include "main.h"

/****** Thread ********/
void CLightWorker::ThreadLightZRPISimplified() {
    RenameThread("rpicoin-light-thread");
    ...
    while (true) {
        try {
            ...
            LogPrintf("%s pop work for %s \n\n", "rpicoin-light-thread", genWit.toString());
            ...
            if (!pIndex) {
                ...
                if (blockHeight >= Params().NEW_PROTOCOLS_STARTHEIGHT()) {
                    ...
                    try {
                        ...
                        LogPrintStr(std::string("ThreadLightZRPISimplified: ") + e.message + "\n");
                    ...
                    if (!res) {
                        ...
                        ss << heightStop;
                        if (genWit.getPfrom()) {
                            LogPrintf("%s pushing message to %s \n", "rpicoin-light-thread", genWit.getPfrom()->addrName);
            ...
            PrintExceptionContinue(&e, "lightzrpithread");
            break;
        }
    }
...
void CLightWorker::rejectWork(CGenWit& wit, int blockHeight, uint32_t errorNumber) {
    if (wit.getStartingHeight() == blockHeight){
        LogPrintf("%s rejecting work %s , error code: %s\n", "rpicoin-light-thread", wit.toString(), errorNumber);
        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
