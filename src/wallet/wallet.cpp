...
bool CWallet::MintableCoins()
{
    ...
    if (nBalance > 0) {
        ...
        for (const COutput& out : vCoins) {
            ...
            if (Params().NetworkID() == CBaseChainParams::REGTEST || GetAdjustedTime() - nTxTime >= GetStakeMinAge())
                return true;
        }
    }
...
bool CWallet::GetBudgetSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX_OLD)); // Old 50 RPI collateral
...
bool CWallet::GetBudgetFinalizationCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    ...
    std::string strFail = "";
    std::vector<std::pair<CScript, CAmount> > vecSend;
    vecSend.push_back(std::make_pair(scriptChange, BUDGET_FEE_TX)); // New 5 RPI collateral
...
std::string CWallet::GetUniqueWalletBackupName(bool fzrpiAuto) const
{
...
void CWallet::ZRpiBackupWallet()
{
    ...
    for (int i = 0; i < 10; i++) {
        ...
        if (boost::filesystem::exists(backupPath)) {
            ...
            if (i <= 8) {
                ...
                boost::filesystem::path nextBackupPath = backupDir / strprintf("wallet-autozrpibackup-%d.dat", i + 1);
    ...
    if(!GetArg("-zrpibackuppath", "").empty()) {
        boost::filesystem::path customPath(GetArg("-zrpibackuppath", ""));
